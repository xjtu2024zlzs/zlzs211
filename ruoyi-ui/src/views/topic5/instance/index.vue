    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          icon="Plus"
          @click="handleAdd"
          v-hasPermi="['topic5:instance:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['topic5:instance:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['topic5:instance:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          icon="Download"
          @click="handleExport"
          v-hasPermi="['topic5:instance:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="instanceList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['topic5:instance:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['topic5:instance:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
import { listInstance, getInstance, delInstance, addInstance, updateInstance } from "@/api/topic5/instance"
...
function handleUpdate(row) {
  ...
  getInstance(_partId).then(response => {
    form.value = response.data
    open.value = true
    title.value = "修改课题五-零部件实例"
  })
}
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除课题五-零部件实例编号为"' + _partIds + '"的数据项？').then(function() {
    return delInstance(_partIds)
  }).then(() => {
    getList()
...
function handleExport() {
  proxy.download('topic5/instance/export', {
    ...queryParams.value
