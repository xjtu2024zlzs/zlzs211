...
-- Description: 课题四增量数据库脚本，包含课题四菜单权限、业务表结构与演示数据。
-- Database: ry-cloud
--
-- 导入顺序：
--   1. 先导入主课题 / RuoYi 基础数据库 SQL，确保 sys_menu 等基础表存在。
--   2. 再导入本文件 project4_data.sql。
--
-- 说明：
--   1. 本文件不包含 RuoYi 主系统基础表，例如 sys_user、sys_role 等。
--   2. 本文件包含课题四 sys_menu 菜单与按钮权限增量 SQL，依赖主系统已存在 sys_menu 表。
--   3. 菜单插入采用 NOT EXISTS 判断，重复执行不会重复插入同 path/component/perms 的菜单项。
--   4. 业务表部分采用 DROP TABLE IF EXISTS + CREATE TABLE，用于演示环境初始化；
--      如果主课题要求保留已有业务数据，请删除 DROP TABLE 段后再执行。
--   5. 本脚本不包含数据库账号、密码、服务器地址等敏感信息。
-- ============================================================
...
-- ============================================================
-- 1. 课题四菜单与按钮权限 SQL
-- 说明：本节依赖 RuoYi 基础表 sys_menu，请先导入主课题基础数据库 SQL。
-- ============================================================

-- 1.1 课题四一级目录
SET @topic4_parent_id := (
  SELECT menu_id FROM sys_menu
  WHERE menu_name = '课题四' AND menu_type = 'M'
...
SELECT
  '课题四', 0, 4, 'project4', 'Layout', NULL, 1, 0, 'M', '0', '0', NULL, 'education', 'admin', NOW(), '课题四目录菜单'
WHERE @topic4_parent_id IS NULL;
...
INSERT INTO sys_menu
  (`menu_name`, `parent_id`, `order_num`, `path`, `component`, `query`, `is_frame`, `is_cache`, `menu_type`, `visible`, `status`, `perms`, `icon`, `create_by`, `create_time`, `remark`)
SELECT
  '数据文件管理', @topic4_parent_id, 1, 'fileofda', 'system/fileofda/index', NULL, 1, 0, 'C', '0', '0', 'system:fileofda:list', 'file', 'admin', NOW(), '课题四数据文件管理页面'
WHERE NOT EXISTS (
  SELECT 1 FROM sys_menu WHERE parent_id = @topic4_parent_id AND path = 'fileofda' AND component = 'system/fileofda/index'
);

SET @menu_fileofda_id := (
  SELECT menu_id FROM sys_menu
  WHERE parent_id = @topic4_parent_id AND path = 'fileofda' AND component = 'system/fileofda/index'
  ORDER BY menu_id DESC
  LIMIT 1
);
...
INSERT INTO sys_menu
  (`menu_name`, `parent_id`, `order_num`, `path`, `component`, `query`, `is_frame`, `is_cache`, `menu_type`, `visible`, `status`, `perms`, `icon`, `create_by`, `create_time`, `remark`)
SELECT
  '样本增强', @topic4_parent_id, 2, 'resultofen', 'system/resultofen/index', NULL, 1, 0, 'C', '0', '0', 'system:resultofen:list', 'example', 'admin', NOW(), '课题四样本增强页面'
WHERE NOT EXISTS (
  SELECT 1 FROM sys_menu WHERE parent_id = @topic4_parent_id AND path = 'resultofen' AND component = 'system/resultofen/index'
);

SET @menu_resultofen_id := (
  SELECT menu_id FROM sys_menu
  WHERE parent_id = @topic4_parent_id AND path = 'resultofen' AND component = 'system/resultofen/index'
  ORDER BY menu_id DESC
  LIMIT 1
);
...
INSERT INTO sys_menu
  (`menu_name`, `parent_id`, `order_num`, `path`, `component`, `query`, `is_frame`, `is_cache`, `menu_type`, `visible`, `status`, `perms`, `icon`, `create_by`, `create_time`, `remark`)
SELECT
  '特征融合', @topic4_parent_id, 3, 'resultoffu', 'system/resultoffu/index', NULL, 1, 0, 'C', '0', '0', 'system:resultoffu:list', 'chart', 'admin', NOW(), '课题四特征融合页面'
WHERE NOT EXISTS (
  SELECT 1 FROM sys_menu WHERE parent_id = @topic4_parent_id AND path = 'resultoffu' AND component = 'system/resultoffu/index'
);

SET @menu_resultoffu_id := (
  SELECT menu_id FROM sys_menu
  WHERE parent_id = @topic4_parent_id AND path = 'resultoffu' AND component = 'system/resultoffu/index'
  ORDER BY menu_id DESC
  LIMIT 1
);
...
INSERT INTO sys_menu
  (`menu_name`, `parent_id`, `order_num`, `path`, `component`, `query`, `is_frame`, `is_cache`, `menu_type`, `visible`, `status`, `perms`, `icon`, `create_by`, `create_time`, `remark`)
SELECT
  '诊断结果', @topic4_parent_id, 4, 'resultofgr', 'system/resultofgr/index', NULL, 1, 0, 'C', '0', '0', 'system:resultofgr:list', 'monitor', 'admin', NOW(), '课题四故障诊断结果页面'
WHERE NOT EXISTS (
  SELECT 1 FROM sys_menu WHERE parent_id = @topic4_parent_id AND path = 'resultofgr' AND component = 'system/resultofgr/index'
);

SET @menu_resultofgr_id := (
  SELECT menu_id FROM sys_menu
  WHERE parent_id = @topic4_parent_id AND path = 'resultofgr' AND component = 'system/resultofgr/index'
  ORDER BY menu_id DESC
  LIMIT 1
);
...
INSERT INTO sys_menu
  (`menu_name`, `parent_id`, `order_num`, `path`, `component`, `query`, `is_frame`, `is_cache`, `menu_type`, `visible`, `status`, `perms`, `icon`, `create_by`, `create_time`, `remark`)
SELECT
  '根因分析', @topic4_parent_id, 5, 'resultofrc', 'system/resultofrc/index', NULL, 1, 0, 'C', '0', '0', 'system:resultofrc:list', 'tree', 'admin', NOW(), '课题四根因分析页面'
WHERE NOT EXISTS (
  SELECT 1 FROM sys_menu WHERE parent_id = @topic4_parent_id AND path = 'resultofrc' AND component = 'system/resultofrc/index'
);

SET @menu_resultofrc_id := (
  SELECT menu_id FROM sys_menu
  WHERE parent_id = @topic4_parent_id AND path = 'resultofrc' AND component = 'system/resultofrc/index'
  ORDER BY menu_id DESC
  LIMIT 1
);
...
SELECT '数据文件查询', @menu_fileofda_id, 1, '', '', NULL, 1, 0, 'F', '0', '0', 'system:fileofda:query', '#', 'admin', NOW(), '数据文件查询按钮权限'
WHERE NOT EXISTS (SELECT 1 FROM sys_menu WHERE perms = 'system:fileofda:query');
INSERT INTO sys_menu (`menu_name`, `parent_id`, `order_num`, `path`, `component`, `query`, `is_frame`, `is_cache`, `menu_type`, `visible`, `status`, `perms`, `icon`, `create_by`, `create_time`, `remark`)
SELECT '数据文件新增', @menu_fileofda_id, 2, '', '', NULL, 1, 0, 'F', '0', '0', 'system:fileofda:add', '#', 'admin', NOW(), '数据文件新增按钮权限'
WHERE NOT EXISTS (SELECT 1 FROM sys_menu WHERE perms = 'system:fileofda:add');
INSERT INTO sys_menu (`menu_name`, `parent_id`, `order_num`, `path`, `component`, `query`, `is_frame`, `is_cache`, `menu_type`, `visible`, `status`, `perms`, `icon`, `create_by`, `create_time`, `remark`)
SELECT '数据文件修改', @menu_fileofda_id, 3, '', '', NULL, 1, 0, 'F', '0', '0', 'system:fileofda:edit', '#', 'admin', NOW(), '数据文件修改按钮权限'
WHERE NOT EXISTS (SELECT 1 FROM sys_menu WHERE perms = 'system:fileofda:edit');
INSERT INTO sys_menu (`menu_name`, `parent_id`, `order_num`, `path`, `component`, `query`, `is_frame`, `is_cache`, `menu_type`, `visible`, `status`, `perms`, `icon`, `create_by`, `create_time`, `remark`)
SELECT '数据文件删除', @menu_fileofda_id, 4, '', '', NULL, 1, 0, 'F', '0', '0', 'system:fileofda:remove', '#', 'admin', NOW(), '数据文件删除按钮权限'
WHERE NOT EXISTS (SELECT 1 FROM sys_menu WHERE perms = 'system:fileofda:remove');
INSERT INTO sys_menu (`menu_name`, `parent_id`, `order_num`, `path`, `component`, `query`, `is_frame`, `is_cache`, `menu_type`, `visible`, `status`, `perms`, `icon`, `create_by`, `create_time`, `remark`)
SELECT '数据文件导出', @menu_fileofda_id, 5, '', '', NULL, 1, 0, 'F', '0', '0', 'system:fileofda:export', '#', 'admin', NOW(), '数据文件导出按钮权限'
WHERE NOT EXISTS (SELECT 1 FROM sys_menu WHERE perms = 'system:fileofda:export');
INSERT INTO sys_menu (`menu_name`, `parent_id`, `order_num`, `path`, `component`, `query`, `is_frame`, `is_cache`, `menu_type`, `visible`, `status`, `perms`, `icon`, `create_by`, `create_time`, `remark`)
SELECT '执行数据预处理', @menu_fileofda_id, 6, '', '', NULL, 1, 0, 'F', '0', '0', 'system:fileofda:preprocess', '#', 'admin', NOW(), '执行数据预处理按钮权限'
WHERE NOT EXISTS (SELECT 1 FROM sys_menu WHERE perms = 'system:fileofda:preprocess');

-- 1.8 样本增强按钮权限
INSERT INTO sys_menu (`menu_name`, `parent_id`, `order_num`, `path`, `component`, `query`, `is_frame`, `is_cache`, `menu_type`, `visible`, `status`, `perms`, `icon`, `create_by`, `create_time`, `remark`)
SELECT '样本增强查询', @menu_resultofen_id, 1, '', '', NULL, 1, 0, 'F', '0', '0', 'system:resultofen:query', '#', 'admin', NOW(), '样本增强查询按钮权限'
WHERE NOT EXISTS (SELECT 1 FROM sys_menu WHERE perms = 'system:resultofen:query');
INSERT INTO sys_menu (`menu_name`, `parent_id`, `order_num`, `path`, `component`, `query`, `is_frame`, `is_cache`, `menu_type`, `visible`, `status`, `perms`, `icon`, `create_by`, `create_time`, `remark`)
SELECT '样本增强新增', @menu_resultofen_id, 2, '', '', NULL, 1, 0, 'F', '0', '0', 'system:resultofen:add', '#', 'admin', NOW(), '样本增强新增按钮权限'
WHERE NOT EXISTS (SELECT 1 FROM sys_menu WHERE perms = 'system:resultofen:add');
INSERT INTO sys_menu (`menu_name`, `parent_id`, `order_num`, `path`, `component`, `query`, `is_frame`, `is_cache`, `menu_type`, `visible`, `status`, `perms`, `icon`, `create_by`, `create_time`, `remark`)
SELECT '样本增强修改', @menu_resultofen_id, 3, '', '', NULL, 1, 0, 'F', '0', '0', 'system:resultofen:edit', '#', 'admin', NOW(), '样本增强修改按钮权限'
WHERE NOT EXISTS (SELECT 1 FROM sys_menu WHERE perms = 'system:resultofen:edit');
INSERT INTO sys_menu (`menu_name`, `parent_id`, `order_num`, `path`, `component`, `query`, `is_frame`, `is_cache`, `menu_type`, `visible`, `status`, `perms`, `icon`, `create_by`, `create_time`, `remark`)
SELECT '样本增强删除', @menu_resultofen_id, 4, '', '', NULL, 1, 0, 'F', '0', '0', 'system:resultofen:remove', '#', 'admin', NOW(), '样本增强删除按钮权限'
WHERE NOT EXISTS (SELECT 1 FROM sys_menu WHERE perms = 'system:resultofen:remove');
INSERT INTO sys_menu (`menu_name`, `parent_id`, `order_num`, `path`, `component`, `query`, `is_frame`, `is_cache`, `menu_type`, `visible`, `status`, `perms`, `icon`, `create_by`, `create_time`, `remark`)
SELECT '样本增强导出', @menu_resultofen_id, 5, '', '', NULL, 1, 0, 'F', '0', '0', 'system:resultofen:export', '#', 'admin', NOW(), '样本增强导出按钮权限'
WHERE NOT EXISTS (SELECT 1 FROM sys_menu WHERE perms = 'system:resultofen:export');
INSERT INTO sys_menu (`menu_name`, `parent_id`, `order_num`, `path`, `component`, `query`, `is_frame`, `is_cache`, `menu_type`, `visible`, `status`, `perms`, `icon`, `create_by`, `create_time`, `remark`)
SELECT '执行样本增强', @menu_resultofen_id, 6, '', '', NULL, 1, 0, 'F', '0', '0', 'system:resultofen:execute', '#', 'admin', NOW(), '执行样本增强按钮权限'
WHERE NOT EXISTS (SELECT 1 FROM sys_menu WHERE perms = 'system:resultofen:execute');

-- 1.9 特征融合按钮权限
INSERT INTO sys_menu (`menu_name`, `parent_id`, `order_num`, `path`, `component`, `query`, `is_frame`, `is_cache`, `menu_type`, `visible`, `status`, `perms`, `icon`, `create_by`, `create_time`, `remark`)
SELECT '特征融合查询', @menu_resultoffu_id, 1, '', '', NULL, 1, 0, 'F', '0', '0', 'system:resultoffu:query', '#', 'admin', NOW(), '特征融合查询按钮权限'
WHERE NOT EXISTS (SELECT 1 FROM sys_menu WHERE perms = 'system:resultoffu:query');
INSERT INTO sys_menu (`menu_name`, `parent_id`, `order_num`, `path`, `component`, `query`, `is_frame`, `is_cache`, `menu_type`, `visible`, `status`, `perms`, `icon`, `create_by`, `create_time`, `remark`)
SELECT '特征融合新增', @menu_resultoffu_id, 2, '', '', NULL, 1, 0, 'F', '0', '0', 'system:resultoffu:add', '#', 'admin', NOW(), '特征融合新增按钮权限'
WHERE NOT EXISTS (SELECT 1 FROM sys_menu WHERE perms = 'system:resultoffu:add');
INSERT INTO sys_menu (`menu_name`, `parent_id`, `order_num`, `path`, `component`, `query`, `is_frame`, `is_cache`, `menu_type`, `visible`, `status`, `perms`, `icon`, `create_by`, `create_time`, `remark`)
SELECT '特征融合修改', @menu_resultoffu_id, 3, '', '', NULL, 1, 0, 'F', '0', '0', 'system:resultoffu:edit', '#', 'admin', NOW(), '特征融合修改按钮权限'
WHERE NOT EXISTS (SELECT 1 FROM sys_menu WHERE perms = 'system:resultoffu:edit');
INSERT INTO sys_menu (`menu_name`, `parent_id`, `order_num`, `path`, `component`, `query`, `is_frame`, `is_cache`, `menu_type`, `visible`, `status`, `perms`, `icon`, `create_by`, `create_time`, `remark`)
SELECT '特征融合删除', @menu_resultoffu_id, 4, '', '', NULL, 1, 0, 'F', '0', '0', 'system:resultoffu:remove', '#', 'admin', NOW(), '特征融合删除按钮权限'
WHERE NOT EXISTS (SELECT 1 FROM sys_menu WHERE perms = 'system:resultoffu:remove');
INSERT INTO sys_menu (`menu_name`, `parent_id`, `order_num`, `path`, `component`, `query`, `is_frame`, `is_cache`, `menu_type`, `visible`, `status`, `perms`, `icon`, `create_by`, `create_time`, `remark`)
SELECT '特征融合导出', @menu_resultoffu_id, 5, '', '', NULL, 1, 0, 'F', '0', '0', 'system:resultoffu:export', '#', 'admin', NOW(), '特征融合导出按钮权限'
WHERE NOT EXISTS (SELECT 1 FROM sys_menu WHERE perms = 'system:resultoffu:export');
INSERT INTO sys_menu (`menu_name`, `parent_id`, `order_num`, `path`, `component`, `query`, `is_frame`, `is_cache`, `menu_type`, `visible`, `status`, `perms`, `icon`, `create_by`, `create_time`, `remark`)
SELECT '执行特征融合', @menu_resultoffu_id, 6, '', '', NULL, 1, 0, 'F', '0', '0', 'system:resultoffu:execute', '#', 'admin', NOW(), '执行特征融合按钮权限'
WHERE NOT EXISTS (SELECT 1 FROM sys_menu WHERE perms = 'system:resultoffu:execute');

-- 1.10 诊断结果按钮权限
INSERT INTO sys_menu (`menu_name`, `parent_id`, `order_num`, `path`, `component`, `query`, `is_frame`, `is_cache`, `menu_type`, `visible`, `status`, `perms`, `icon`, `create_by`, `create_time`, `remark`)
SELECT '诊断结果查询', @menu_resultofgr_id, 1, '', '', NULL, 1, 0, 'F', '0', '0', 'system:resultofgr:query', '#', 'admin', NOW(), '诊断结果查询按钮权限'
WHERE NOT EXISTS (SELECT 1 FROM sys_menu WHERE perms = 'system:resultofgr:query');
INSERT INTO sys_menu (`menu_name`, `parent_id`, `order_num`, `path`, `component`, `query`, `is_frame`, `is_cache`, `menu_type`, `visible`, `status`, `perms`, `icon`, `create_by`, `create_time`, `remark`)
SELECT '诊断结果新增', @menu_resultofgr_id, 2, '', '', NULL, 1, 0, 'F', '0', '0', 'system:resultofgr:add', '#', 'admin', NOW(), '诊断结果新增按钮权限'
WHERE NOT EXISTS (SELECT 1 FROM sys_menu WHERE perms = 'system:resultofgr:add');
INSERT INTO sys_menu (`menu_name`, `parent_id`, `order_num`, `path`, `component`, `query`, `is_frame`, `is_cache`, `menu_type`, `visible`, `status`, `perms`, `icon`, `create_by`, `create_time`, `remark`)
SELECT '诊断结果修改', @menu_resultofgr_id, 3, '', '', NULL, 1, 0, 'F', '0', '0', 'system:resultofgr:edit', '#', 'admin', NOW(), '诊断结果修改按钮权限'
WHERE NOT EXISTS (SELECT 1 FROM sys_menu WHERE perms = 'system:resultofgr:edit');
INSERT INTO sys_menu (`menu_name`, `parent_id`, `order_num`, `path`, `component`, `query`, `is_frame`, `is_cache`, `menu_type`, `visible`, `status`, `perms`, `icon`, `create_by`, `create_time`, `remark`)
SELECT '诊断结果删除', @menu_resultofgr_id, 4, '', '', NULL, 1, 0, 'F', '0', '0', 'system:resultofgr:remove', '#', 'admin', NOW(), '诊断结果删除按钮权限'
WHERE NOT EXISTS (SELECT 1 FROM sys_menu WHERE perms = 'system:resultofgr:remove');
INSERT INTO sys_menu (`menu_name`, `parent_id`, `order_num`, `path`, `component`, `query`, `is_frame`, `is_cache`, `menu_type`, `visible`, `status`, `perms`, `icon`, `create_by`, `create_time`, `remark`)
SELECT '诊断结果导出', @menu_resultofgr_id, 5, '', '', NULL, 1, 0, 'F', '0', '0', 'system:resultofgr:export', '#', 'admin', NOW(), '诊断结果导出按钮权限'
WHERE NOT EXISTS (SELECT 1 FROM sys_menu WHERE perms = 'system:resultofgr:export');
INSERT INTO sys_menu (`menu_name`, `parent_id`, `order_num`, `path`, `component`, `query`, `is_frame`, `is_cache`, `menu_type`, `visible`, `status`, `perms`, `icon`, `create_by`, `create_time`, `remark`)
SELECT '执行故障诊断', @menu_resultofgr_id, 6, '', '', NULL, 1, 0, 'F', '0', '0', 'system:resultofgr:execute', '#', 'admin', NOW(), '执行故障诊断按钮权限'
WHERE NOT EXISTS (SELECT 1 FROM sys_menu WHERE perms = 'system:resultofgr:execute');

-- 1.11 根因分析按钮权限
INSERT INTO sys_menu (`menu_name`, `parent_id`, `order_num`, `path`, `component`, `query`, `is_frame`, `is_cache`, `menu_type`, `visible`, `status`, `perms`, `icon`, `create_by`, `create_time`, `remark`)
SELECT '根因分析查询', @menu_resultofrc_id, 1, '', '', NULL, 1, 0, 'F', '0', '0', 'system:resultofrc:query', '#', 'admin', NOW(), '根因分析查询按钮权限'
WHERE NOT EXISTS (SELECT 1 FROM sys_menu WHERE perms = 'system:resultofrc:query');
INSERT INTO sys_menu (`menu_name`, `parent_id`, `order_num`, `path`, `component`, `query`, `is_frame`, `is_cache`, `menu_type`, `visible`, `status`, `perms`, `icon`, `create_by`, `create_time`, `remark`)
SELECT '根因分析新增', @menu_resultofrc_id, 2, '', '', NULL, 1, 0, 'F', '0', '0', 'system:resultofrc:add', '#', 'admin', NOW(), '根因分析新增按钮权限'
WHERE NOT EXISTS (SELECT 1 FROM sys_menu WHERE perms = 'system:resultofrc:add');
INSERT INTO sys_menu (`menu_name`, `parent_id`, `order_num`, `path`, `component`, `query`, `is_frame`, `is_cache`, `menu_type`, `visible`, `status`, `perms`, `icon`, `create_by`, `create_time`, `remark`)
SELECT '根因分析修改', @menu_resultofrc_id, 3, '', '', NULL, 1, 0, 'F', '0', '0', 'system:resultofrc:edit', '#', 'admin', NOW(), '根因分析修改按钮权限'
WHERE NOT EXISTS (SELECT 1 FROM sys_menu WHERE perms = 'system:resultofrc:edit');
INSERT INTO sys_menu (`menu_name`, `parent_id`, `order_num`, `path`, `component`, `query`, `is_frame`, `is_cache`, `menu_type`, `visible`, `status`, `perms`, `icon`, `create_by`, `create_time`, `remark`)
SELECT '根因分析删除', @menu_resultofrc_id, 4, '', '', NULL, 1, 0, 'F', '0', '0', 'system:resultofrc:remove', '#', 'admin', NOW(), '根因分析删除按钮权限'
WHERE NOT EXISTS (SELECT 1 FROM sys_menu WHERE perms = 'system:resultofrc:remove');
INSERT INTO sys_menu (`menu_name`, `parent_id`, `order_num`, `path`, `component`, `query`, `is_frame`, `is_cache`, `menu_type`, `visible`, `status`, `perms`, `icon`, `create_by`, `create_time`, `remark`)
SELECT '根因分析导出', @menu_resultofrc_id, 5, '', '', NULL, 1, 0, 'F', '0', '0', 'system:resultofrc:export', '#', 'admin', NOW(), '根因分析导出按钮权限'
WHERE NOT EXISTS (SELECT 1 FROM sys_menu WHERE perms = 'system:resultofrc:export');
INSERT INTO sys_menu (`menu_name`, `parent_id`, `order_num`, `path`, `component`, `query`, `is_frame`, `is_cache`, `menu_type`, `visible`, `status`, `perms`, `icon`, `create_by`, `create_time`, `remark`)
SELECT '执行根因分析', @menu_resultofrc_id, 6, '', '', NULL, 1, 0, 'F', '0', '0', 'system:resultofrc:execute', '#', 'admin', NOW(), '执行根因分析按钮权限'
WHERE NOT EXISTS (SELECT 1 FROM sys_menu WHERE perms = 'system:resultofrc:execute');

-- 1.12 可选：普通角色菜单授权
-- 说明：
--   admin 超级管理员通常无需配置 sys_role_menu。
--   如果使用普通角色登录，请在系统后台“角色管理”中给对应角色勾选“课题四”菜单。
--   不建议在共享脚本中写死 role_id，避免污染主课题权限配置。
--
-- 示例，仅供需要时手动修改后执行：
...
-- ============================================================
-- 2. 清理课题四旧表
-- 说明：演示初始化脚本会清空并重建课题四业务表。
-- 如需保留已有课题四业务数据，请删除本节 DROP TABLE 语句。
-- ============================================================
...
CREATE TABLE `fd_analysis_pipeline` (
  ...
  CONSTRAINT `fk_fd_pipeline_dataset_id` FOREIGN KEY (`dataset_id`) REFERENCES `fd_dataset_info` (`dataset_id`)
...
CREATE TABLE `fd_data_file` (
  ...
  CONSTRAINT `fk_fd_file_dataset_id` FOREIGN KEY (`dataset_id`) REFERENCES `fd_dataset_info` (`dataset_id`),
  CONSTRAINT `fk_fd_file_pipeline_id` FOREIGN KEY (`pipeline_id`) REFERENCES `fd_analysis_pipeline` (`pipeline_id`)
...
CREATE TABLE `fd_processed_sample` (
  ...
  CONSTRAINT `fk_fd_processed_pipeline_id` FOREIGN KEY (`pipeline_id`) REFERENCES `fd_analysis_pipeline` (`pipeline_id`),
  CONSTRAINT `fk_fd_processed_dataset_id` FOREIGN KEY (`dataset_id`) REFERENCES `fd_dataset_info` (`dataset_id`),
  CONSTRAINT `fk_fd_processed_file_id` FOREIGN KEY (`file_id`) REFERENCES `fd_data_file` (`file_id`)
...
CREATE TABLE `fd_augment_result` (
  ...
  CONSTRAINT `fk_fd_augment_pipeline_id` FOREIGN KEY (`pipeline_id`) REFERENCES `fd_analysis_pipeline` (`pipeline_id`),
  CONSTRAINT `fk_fd_augment_dataset_id` FOREIGN KEY (`dataset_id`) REFERENCES `fd_dataset_info` (`dataset_id`),
  CONSTRAINT `fk_fd_augment_sample_id` FOREIGN KEY (`sample_id`) REFERENCES `fd_processed_sample` (`sample_id`)
...
CREATE TABLE `fd_fusion_result` (
  ...
  CONSTRAINT `fk_fd_fusion_pipeline_id` FOREIGN KEY (`pipeline_id`) REFERENCES `fd_analysis_pipeline` (`pipeline_id`),
  CONSTRAINT `fk_fd_fusion_dataset_id` FOREIGN KEY (`dataset_id`) REFERENCES `fd_dataset_info` (`dataset_id`),
  CONSTRAINT `fk_fd_fusion_sample_id` FOREIGN KEY (`sample_id`) REFERENCES `fd_processed_sample` (`sample_id`),
  CONSTRAINT `fk_fd_fusion_augment_id` FOREIGN KEY (`augment_id`) REFERENCES `fd_augment_result` (`augment_id`)
...
CREATE TABLE `fd_diagnosis_result` (
  ...
  CONSTRAINT `fk_fd_diagnosis_pipeline_id` FOREIGN KEY (`pipeline_id`) REFERENCES `fd_analysis_pipeline` (`pipeline_id`),
  CONSTRAINT `fk_fd_diagnosis_dataset_id` FOREIGN KEY (`dataset_id`) REFERENCES `fd_dataset_info` (`dataset_id`),
  CONSTRAINT `fk_fd_diagnosis_sample_id` FOREIGN KEY (`sample_id`) REFERENCES `fd_processed_sample` (`sample_id`),
  CONSTRAINT `fk_fd_diagnosis_fusion_id` FOREIGN KEY (`fusion_id`) REFERENCES `fd_fusion_result` (`fusion_id`)
...
CREATE TABLE `fd_root_cause_analysis` (
  ...
  CONSTRAINT `fk_fd_root_pipeline_id` FOREIGN KEY (`pipeline_id`) REFERENCES `fd_analysis_pipeline` (`pipeline_id`),
  CONSTRAINT `fk_fd_root_dataset_id` FOREIGN KEY (`dataset_id`) REFERENCES `fd_dataset_info` (`dataset_id`),
  CONSTRAINT `fk_fd_root_sample_id` FOREIGN KEY (`sample_id`) REFERENCES `fd_processed_sample` (`sample_id`),
  CONSTRAINT `fk_fd_root_diagnosis_id` FOREIGN KEY (`diagnosis_id`) REFERENCES `fd_diagnosis_result` (`diagnosis_id`)
