-- designtask1 module startup fix for t2_ tables
-- Execute this script in the business database used by ruoyi-system-dev.yml, usually `ry-cloud`.
-- It is safe to run more than once: existing data is preserved.

SET NAMES utf8mb4;
SET FOREIGN_KEY_CHECKS = 0;

DROP PROCEDURE IF EXISTS t2_rename_table_if_exists;
DELIMITER $$
CREATE PROCEDURE t2_rename_table_if_exists(
...
CALL t2_rename_table_if_exists('design_task', 't2_design_task');
CALL t2_rename_table_if_exists('design_template', 't2_design_template');
CALL t2_rename_table_if_exists('design_template_node', 't2_design_template_node');
CALL t2_rename_table_if_exists('design_template_node_role', 't2_design_template_node_role');
CALL t2_rename_table_if_exists('design_task_node', 't2_design_task_node');
CALL t2_rename_table_if_exists('design_task_file', 't2_design_task_file');
CALL t2_rename_table_if_exists('design_task_log', 't2_design_task_log');
CALL t2_rename_table_if_exists('design_objective_catalog', 't2_design_objective_catalog');
CALL t2_rename_table_if_exists('design_objective_constraint', 't2_design_objective_constraint');
CALL t2_rename_table_if_exists('design_variable_catalog', 't2_design_variable_catalog');
CALL t2_rename_table_if_exists('design_task_variable_selection', 't2_design_task_variable_selection');
CALL t2_rename_table_if_exists('design_conflict_check', 't2_design_conflict_check');
CALL t2_rename_table_if_exists('design_subtask_solution', 't2_design_subtask_solution');
CALL t2_rename_table_if_exists('design_simulation_result', 't2_design_simulation_result');
CALL t2_rename_table_if_exists('design_approval_record', 't2_design_approval_record');
CALL t2_rename_table_if_exists('design_resource', 't2_design_resource');
CALL t2_rename_table_if_exists('surrogate_solve_task', 't2_surrogate_solve_task');
CALL t2_rename_table_if_exists('cad_model_task', 't2_cad_model_task');

CALL t2_rename_table_if_exists('p2_design_task', 't2_design_task');
CALL t2_rename_table_if_exists('p2_design_template', 't2_design_template');
CALL t2_rename_table_if_exists('p2_design_template_node', 't2_design_template_node');
CALL t2_rename_table_if_exists('p2_design_template_node_role', 't2_design_template_node_role');
CALL t2_rename_table_if_exists('p2_design_task_node', 't2_design_task_node');
CALL t2_rename_table_if_exists('p2_design_task_file', 't2_design_task_file');
CALL t2_rename_table_if_exists('p2_design_task_log', 't2_design_task_log');
CALL t2_rename_table_if_exists('p2_design_objective_catalog', 't2_design_objective_catalog');
CALL t2_rename_table_if_exists('p2_design_objective_constraint', 't2_design_objective_constraint');
CALL t2_rename_table_if_exists('p2_design_variable_catalog', 't2_design_variable_catalog');
CALL t2_rename_table_if_exists('p2_design_task_variable_selection', 't2_design_task_variable_selection');
CALL t2_rename_table_if_exists('p2_design_conflict_check', 't2_design_conflict_check');
CALL t2_rename_table_if_exists('p2_design_subtask_solution', 't2_design_subtask_solution');
CALL t2_rename_table_if_exists('p2_design_simulation_result', 't2_design_simulation_result');
CALL t2_rename_table_if_exists('p2_design_approval_record', 't2_design_approval_record');
CALL t2_rename_table_if_exists('p2_design_resource', 't2_design_resource');
CALL t2_rename_table_if_exists('p2_surrogate_solve_task', 't2_surrogate_solve_task');
CALL t2_rename_table_if_exists('p2_cad_model_task', 't2_cad_model_task');
...
CALL t2_add_column_if_missing('t2_design_task', 'process_definition_id', "varchar(128) DEFAULT NULL COMMENT 'Flowable process definition ID'");
CALL t2_add_column_if_missing('t2_design_task', 'process_instance_id', "varchar(128) DEFAULT NULL COMMENT 'Flowable process instance ID'");
CALL t2_add_column_if_missing('t2_design_task', 'current_flowable_task_id', "varchar(128) DEFAULT NULL COMMENT 'Current Flowable task ID'");
CALL t2_add_column_if_missing('t2_design_task', 'current_node_key', "varchar(100) DEFAULT NULL COMMENT 'Current node key'");
CALL t2_add_column_if_missing('t2_design_task', 'current_node_name', "varchar(100) DEFAULT NULL COMMENT 'Current node name'");
CALL t2_add_column_if_missing('t2_design_task', 'owner_user_id', "bigint(20) DEFAULT NULL COMMENT 'Owner user ID'");
CALL t2_add_column_if_missing('t2_design_task', 'owner_user_name', "varchar(100) DEFAULT NULL COMMENT 'Owner user name'");
CALL t2_add_column_if_missing('t2_design_task', 'structure_user_id', "bigint(20) DEFAULT NULL COMMENT 'Structure engineer user ID'");
CALL t2_add_column_if_missing('t2_design_task', 'layout_user_id', "bigint(20) DEFAULT NULL COMMENT 'Layout engineer user ID'");
CALL t2_add_column_if_missing('t2_design_task', 'aero_user_id', "bigint(20) DEFAULT NULL COMMENT 'Aero engineer user ID'");
CALL t2_add_column_if_missing('t2_design_task', 'hydraulic_user_id', "bigint(20) DEFAULT NULL COMMENT 'Hydraulic engineer user ID'");
CALL t2_add_column_if_missing('t2_design_task', 'manufacturing_user_id', "bigint(20) DEFAULT NULL COMMENT 'Manufacturing engineer user ID'");
CALL t2_add_column_if_missing('t2_design_task', 'design_user_id', "bigint(20) DEFAULT NULL COMMENT 'Design engineer user ID'");
CALL t2_add_column_if_missing('t2_design_task', 'leader_user_id', "bigint(20) DEFAULT NULL COMMENT 'Approval leader user ID'");
CALL t2_add_column_if_missing('t2_design_task', 'plan_start_time', "datetime DEFAULT NULL COMMENT 'Planned start time'");
CALL t2_add_column_if_missing('t2_design_task', 'expected_end_time', "datetime DEFAULT NULL COMMENT 'Expected end time'");
...
CALL t2_add_column_if_missing('t2_design_task_file', 'file_size', "bigint(20) DEFAULT NULL COMMENT 'File size'");
CALL t2_add_column_if_missing('t2_design_task_file', 'file_type', "varchar(128) DEFAULT NULL COMMENT 'File type'");
CALL t2_add_column_if_missing('t2_design_task_file', 'file_suffix', "varchar(64) DEFAULT NULL COMMENT 'File suffix'");
CALL t2_add_column_if_missing('t2_design_task_file', 'upload_by', "varchar(64) DEFAULT '' COMMENT 'Uploaded by'");
CALL t2_add_column_if_missing('t2_design_task_file', 'upload_time', "datetime DEFAULT NULL COMMENT 'Uploaded time'");
...
SET @fault_pipe_default_set_id = (SELECT parameter_set_id FROM t2_design_fault_pipe_parameter_set WHERE set_code = 'HP-PIPE-SEG-001' LIMIT 1);
