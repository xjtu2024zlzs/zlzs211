...
-- Execute this single script for Project 2 database integration on master.
-- Do not execute legacy scripts such as `designtask_full.sql`; those scripts may contain
-- non-standard sys_menu columns such as `parent_name`, which are not present in the
-- current RuoYi-Cloud master schema.
-- This script uses the standard sys_menu columns:
...
-- =====================================================================
-- 00A Cleanup residue from legacy designtask_full.sql
-- =====================================================================
-- `designtask_full.sql` used the old p2_* tables and old menu IDs 2000-2004.
-- It also used the non-standard sys_menu.parent_name column, so it may fail halfway
-- and leave partial tables behind. Clean these residues before applying the master
-- migration. Back up the target database before executing this script.
-- Earlier draft versions of this master script also inserted Project22 frame-beam
-- grouping menus 2630-2643 and t2_frame_beam_* tables. They are removed here because
-- the current master integration only includes the hydraulic pipe optimization flow.

DELETE FROM sys_role_menu WHERE menu_id IN (
  2000, 2001, 2002, 2003, 2004,
  2630, 2631, 2632, 2633, 2634, 2640, 2641, 2642, 2643
);
...
WHERE menu_id IN (
   ...
   OR component IN (
      ...
      'designtask/task/kanban',
      'designtask/task/history',
      'designtask/flow/template',
      'designtask/frameBeam/life-prediction',
      'designtask/frameBeam/decision-advice'
   )
   OR path IN (
      'designtask',
      'task/create',
...
-- Drop p2_* tables that remain after the rename pass. If t2_* already existed,
-- the p2_* tables are treated as residue from the legacy script.
DROP TABLE IF EXISTS p2_cad_model_task;
DROP TABLE IF EXISTS p2_surrogate_solve_task;
DROP TABLE IF EXISTS p2_design_resource;
DROP TABLE IF EXISTS p2_design_approval_record;
DROP TABLE IF EXISTS p2_design_simulation_result;
DROP TABLE IF EXISTS p2_design_subtask_solution;
DROP TABLE IF EXISTS p2_design_conflict_check;
DROP TABLE IF EXISTS p2_design_task_variable_selection;
DROP TABLE IF EXISTS p2_design_variable_catalog;
DROP TABLE IF EXISTS p2_design_objective_constraint;
DROP TABLE IF EXISTS p2_design_objective_catalog;
...
-- Drop Project22 frame-beam tables created by earlier draft migration scripts.
-- Keep this cleanup until Project22 is formally integrated by a separate migration.
DROP TABLE IF EXISTS t2_frame_beam_maintenance_advice;
DROP TABLE IF EXISTS t2_frame_beam_life_prediction;
DROP TABLE IF EXISTS t2_frame_beam_load_spectrum;
DROP TABLE IF EXISTS t2_frame_beam_crack_input;
...
VALUES
  (2600, '设计制造协同优化平台', 0, 5, 'designtask', NULL, '', '',
...
-- =====================================================================
-- 08 Remove standard resource menu from design platform
-- =====================================================================
