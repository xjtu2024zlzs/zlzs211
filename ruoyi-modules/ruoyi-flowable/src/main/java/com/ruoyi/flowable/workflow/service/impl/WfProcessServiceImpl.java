...
public class WfProcessServiceImpl extends FlowServiceFactory implements IWfProcessService {
    ...
    public TableDataInfo<WfTaskVo> selectPageOwnProcessList(ProcessQuery processQuery, PageQuery pageQuery) {
        ...
        for (HistoricProcessInstance hisIns : historicProcessInstances) {
            ...
            if (Objects.nonNull(hisIns.getEndTime())) {
                taskVo.setDuration(DateUtils.timeDistance(hisIns.getEndTime(), hisIns.getStartTime()));
            } else {
                taskVo.setDuration(DateUtils.timeDistance(DateUtils.getNowDate(), hisIns.getStartTime()));
            }
            // 流程部署实例信息
    ...
    public TableDataInfo<WfTaskVo> selectPageTodoProcessList(ProcessQuery processQuery, PageQuery pageQuery) {
        ...
        for (Task task : taskList) {
            ...
            R<SysUser> user = userService.getUserInfoByUserId(userId, SecurityConstants.INNER);
...
//        result.put("finished",true);
    ...
    public FormConf selectFormContent(String definitionId, String deployId, String procInsId) {
        ...
        if (ObjectUtil.isNull(deployForm) || StrUtil.isBlank(deployForm.getContent())) {
            FormConf formConf = new FormConf();
            formConf.setFormBtns(false);
            formConf.setFormModel(emptyFormModel());
            formConf.setFormData(new HashMap<>());
            return formConf;
        }
    ...
    private Map<String, Object> emptyFormModel() {
        Map<String, Object> formModel = new HashMap<>();
        formModel.put("formConfig", new HashMap<>());
        formModel.put("widgetList", new ArrayList<>());
        return formModel;
    }
                    ...
                    if ("candidate".equals(identityLink.getType())) {
                        ...
                        if (StringUtils.isNotBlank(identityLink.getGroupId())) {
                            if (identityLink.getGroupId().startsWith(TaskConstants.ROLE_GROUP_PREFIX)) {
                                ...
                                //finish do 查询角色名称
                                R<SysRole> roleR=remoteUserService.getRoleByRoleId(roleId, SecurityConstants.INNER);
                                if (!StringUtils.isNull(roleR) && !StringUtils.isNull(roleR.getData())) {
                                    SysRole role = roleR.getData();
                                ...
                                //finish do 查询部门名称
                                R<SysDept> deptR=remoteUserService.getDeptByDeptId(deptId, SecurityConstants.INNER);
                                if (!StringUtils.isNull(deptR) && !StringUtils.isNull(deptR.getData())) {
