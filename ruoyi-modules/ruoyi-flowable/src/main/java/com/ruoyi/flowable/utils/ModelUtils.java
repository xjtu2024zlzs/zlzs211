...
public class ModelUtils {
    ...
    private static final Pattern XML_PREFIX_PATTERN = Pattern.compile("^[\\uFEFF\\u0000-\\u0008\\u000B\\u000C\\u000E-\\u001F\\s]+");
    ...
    public static BpmnModel getBpmnModel(String xml) {
        return bpmnXMLConverter.convertToBpmnModel(new StringStreamSource(cleanBpmnXml(xml)), false, false);
    }

    public static String cleanBpmnXml(String xml) {
        if (StrUtil.isBlank(xml)) {
            return xml;
        }
        String normalized = xml.replace("\r\n", "\n").replace('\r', '\n');
        normalized = XML_PREFIX_PATTERN.matcher(normalized).replaceFirst("");
        int xmlStart = normalized.indexOf("<?xml");
        int definitionsStart = normalized.indexOf("<bpmn:");
        int bpmn2DefinitionsStart = normalized.indexOf("<bpmn2:");
        int plainDefinitionsStart = normalized.indexOf("<definitions");
        int start = minPositive(xmlStart, definitionsStart, bpmn2DefinitionsStart, plainDefinitionsStart);
        if (start > 0) {
    ...
    public static boolean looksLikeBpmnXml(String xml) {
        String normalized = cleanBpmnXml(xml);
        return StrUtil.isNotBlank(normalized)
            && (normalized.startsWith("<?xml")
            || normalized.startsWith("<bpmn:")
            || normalized.startsWith("<bpmn2:")
    ...
    private static int minPositive(int... values) {
        ...
        for (int value : values) {
            if (value >= 0 && (result == -1 || value < result)) {
                result = value;
            }
        }
        return result;
