...
public class PartQualityTemplateGenerator
{
    ...
    private void designSheet(Workbook workbook, CellStyle headerStyle)
    {
        ...
        addListValidation(sheet, 1, 500, 8, "通过", "不通过", "待评审");
        addListValidation(sheet, 1, 500, 9, "通过", "不通过", "待验证");
    ...
    private void manufacturingSheet(Workbook workbook, CellStyle headerStyle)
    {
        ...
        addListValidation(sheet, 1, 500, 7, "未开始", "进行中", "已完成", "异常", "暂停");
        addListValidation(sheet, 1, 500, 8, "合格", "不合格", "待检");
        autoSize(sheet, headers.length);
    }
    ...
    private void serviceSheet(Workbook workbook, CellStyle headerStyle)
    {
        ...
        String[] headers = {"服役质量ID", "设备ID", "组件ID", "装机飞机ID", "安装位置", "安装日期", "服役状态", "累计运行小时", "循环次数", "健康评分", "上次检修日期", "下次检修日期"};
        writeHeader(sheet, headers, headerStyle);
        writeRow(sheet, 1, "SQ001", "E1", "C1", "A1", "左侧液压舱", "2026-05-20", "在役", "120.5", "30", "98.5", "2026-04-20", "2026-06-20");
        addListValidation(sheet, 1, 500, 6, "在役", "停用", "维修中", "退役");
        autoSize(sheet, headers.length);
    }
