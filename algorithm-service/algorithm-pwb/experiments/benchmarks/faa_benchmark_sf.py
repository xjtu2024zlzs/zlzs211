        ...
        if filename.endswith("_schema.sql"):
            for prefix, name in schema_mapping.items():
                if filename.startswith(prefix):
                    ...
                    schema = parse_schema(filepath, name)
                    schemas[name] = schema
...
def run_benchmark_sf_known_table(
    ...
    # 加载 SQL schema
    schema_dir = os.path.join(project_path, "data", "database", "init-scripts")
    print(f"\n{'='*60}")
...
def run_benchmark_sf_unknown_table(
    ...
    # 加载 SQL schema（自动检测路径）
    # 原始数据集: data/database/init-scripts/
    # FKP 数据集:  data/magneto_faa_fkp/fkp/init-scripts/
    schema_dir_candidates = [
        os.path.join(project_path, ROOT, "fkp", "init-scripts"),   # FKP
        os.path.join(project_path, "data", "database", "init-scripts"),  # 原始
    ]
    schema_dir = None
...
def run_benchmark_global_unknown_table(
    BENCHMARK: str = "faa_fkp_global",
    DATASET: str = "faa_fkp",
    ROOT: str = "data/magneto_faa_fkp",
    MODE: str = "header_values_default",
    embedding_model: str = "mpnet",
    llm_model: str = "none",
    pg_host: str = "localhost",
    ...
    # --- 加载 Schema ---
    schema_dir_candidates = [
        os.path.join(project_path, ROOT, "fkp", "init-scripts"),
        os.path.join(project_path, "data", "database", "init-scripts"),
    ]
    schema_dir = next((c for c in schema_dir_candidates if os.path.isdir(c)), None)
    if schema_dir is None:
        print("错误: 未找到 SQL Schema 目录!")
        return
    ...
    if embedding_model not in model_map and os.path.exists(
        os.path.join(project_path, "models", embedding_model)
    ):
        base_path = model_map["mpnet"]
    ...
    # --- 确定方法列表 ---
    skip_gpt = llm_model is None or llm_model.lower() == "none"
    base_methods = ["Magneto", "MagnetoBoost"]
    if skip_gpt:
        matchers_list = base_methods
    else:
      # gpt_methods = ["MagnetoGPT", "MagnetoGPT_neo4j"]
        gpt_methods = ["MagnetoGPT"]
        if neo4j_graph:
            gpt_methods.append("MagnetoGPT_neo4j2")
        matchers_list = base_methods + gpt_methods
    ...
    for matcher_name in matchers_list:
        ...
        for source_file in pbar:
            ...
            try:
                ...
                elif matcher_name == "MagnetoGPT_neo4j2":
        ...
        if matcher_name in ("MagnetoGPT", "MagnetoGPT_neo4j2"):
...
def main():
    parser = argparse.ArgumentParser(description="运行 FAA 航空数据集 Similarity Flooding 基准测试")
    ...
    parser.add_argument(
        "--root",
        type=str,
        default="data/magneto_faa",
        help="数据集根目录（如 data/magneto_faa 或 data/magneto_faa_fkp）",
    )
    parser.add_argument(
        "--dataset",
        type=str,
        default=None,
        help="数据集名称（默认从 root 推断，如 faa_aviation 或 faa_fkp）",
    )
    parser.add_argument(
        "--pg_host",
        type=str,
